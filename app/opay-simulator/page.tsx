...
export default function OPaySimulatorPage() {
  ...
  useEffect(() => {
    refresh();
    window.addEventListener("fixmate-opay-updated", refresh);
    window.addEventListener("fixmate-db-updated", refresh);
    return () => {
      window.removeEventListener("fixmate-opay-updated", refresh);
      window.removeEventListener("fixmate-db-updated", refresh);
    };
  }, []);
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans pb-16">
      <header className="bg-white px-4 sm:px-6 py-4 flex items-center justify-between border-b shadow-sm">
        <div>
          <div className="text-xs font-bold uppercase tracking-widest text-green-700 mb-0.5">FixMate</div>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <section className="bg-white border border-gray-200 p-5 sm:p-6">
          ...
          <p className="mt-3 text-xs text-gray-500">
            ...
            FixMate verifies the signature, updates the booking, and notifies the artisan.
          </p>
        </section>
