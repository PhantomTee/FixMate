  ...
  useEffect(() => {
    ...
    async function loadUser() {
      ...
      try {
        ...
        setAuthUser({ name: data.user?.name ?? "You", isArtisan: !!data.artisan });
    ...
    <>
      ...
      <header className="sticky top-0 z-50 w-full border-b border-gray-100 bg-white/95 backdrop-blur-sm">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3.5 sm:px-6">
            ...
            {authReady && (
              authUser ? (
                <>
                  ...
                  <Link href="/profile" className="bg-green-700 px-5 py-2 text-sm font-black text-white hover:bg-green-800 transition-colors rounded-lg">
                    My Profile
                  </Link>
          ...
          {/* Mobile: hamburger (auth overflow only) */}
          <button
      ...
      {isOpen && (
        <div className="fixed inset-0 z-40 bg-white pt-16 md:hidden overflow-y-auto pb-20">
          <nav className="flex flex-col text-sm font-semibold text-gray-800">
            {authUser ? (
              <>
                <Link href="/profile" onClick={close} className="border-b border-gray-100 px-6 py-4 hover:bg-gray-50 font-black text-green-700">
                ...
                {/* Notification bell in drawer when logged in */}
                <div className="border-b border-gray-100 px-6 py-4">
                  <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Recent Activity</p>
                  <NotificationList onClose={close} />
                </div>
                <button onClick={signOut}
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-100 grid grid-cols-4 md:hidden" style={{ paddingBottom: "env(safe-area-inset-bottom)" }}>
        <Link href="/"
          className={`flex flex-col items-center gap-0.5 py-2.5 transition-colors ${pathname === "/" ? "text-green-700" : "text-gray-400 hover:text-gray-700"}`}>
          <HomeIcon active={pathname === "/"} />
          <span className="text-[10px] font-black">Home</span>
        </Link>
        <Link href="/browse"
          className={`flex flex-col items-center gap-0.5 py-2.5 transition-colors ${pathname.startsWith("/browse") ? "text-green-700" : "text-gray-400 hover:text-gray-700"}`}>
          <SearchIcon active={pathname.startsWith("/browse")} />
          <span className="text-[10px] font-black">Browse</span>
        </Link>
        <Link href="/report"
          className={`flex flex-col items-center gap-0.5 py-2 mx-2 my-1.5 rounded-xl font-black text-sm transition-colors ${pathname.startsWith("/report") ? "bg-green-800 text-white" : "bg-green-700 text-white hover:bg-green-800"}`}>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
            <line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/>
          </svg>
          <span className="text-[10px] font-black">Hire</span>
        </Link>
        <Link href={authReady && authUser ? "/profile" : "/auth"}
          className={`flex flex-col items-center gap-0.5 py-2.5 transition-colors ${pathname.startsWith("/profile") ? "text-green-700" : "text-gray-400 hover:text-gray-700"}`}>
          <PersonIcon active={pathname.startsWith("/profile")} />
          <span className="text-[10px] font-black">{authReady && authUser ? "Me" : "Sign In"}</span>
        </Link>
      </nav>
    </>
  );
