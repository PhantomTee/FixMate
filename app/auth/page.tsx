...
import { signInWithPhone, verifyOtp } from "@/lib/api";
...
export default function AuthPage() {
  ...
  const sendOtp = async () => {
    ...
    setError("");
    try {
      const { error: err } = await signInWithPhone(phone.trim());
      if (err) throw err;
      setStep("otp");
    } catch (err) {
  ...
  const confirmOtp = async () => {
    ...
    setError("");
    try {
      const { error: err } = await verifyOtp(phone.trim(), otp.trim());
      if (err) throw err;
      router.push(next);
    } catch (err) {
            ...
            <>
              <div>
                ...
                <input
                  ...
                  autoFocus
                />
                <p className="text-[10px] text-gray-400 mt-1 text-center">Sent to {phone}</p>
              </div>
              <button
                onClick={confirmOtp}
